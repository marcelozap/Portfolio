# LinkedIn Format Standard

This is Marcelo's default format for personal LinkedIn writing unless a post
needs a different treatment.

## Format

The caption is the invitation. The attached image is the readable essay.

- Use a black or near-black background with warm white text.
- Use generous margins and phone-readable type.
- Keep the image simple: no dashboard, logo, stock image, or visual clutter.
- Keep the caption short enough to create curiosity without repeating the essay.
- Put the exact public note URL in the caption and as the first comment.

## Writing

- One central idea per post.
- Open with one strong personal sentence.
- Use short paragraphs and lived details.
- Connect AI to a human question, not product hype.
- Keep Marcelo's voice reflective, direct, curious, and willing to test its own assumptions.
- Let the ending be earned; do not add a generic call to action.

## Workflow

1. Write and edit the longer note first.
2. Turn the final note into the black-background image.
3. Write the shorter caption as the entry point.
4. Publish the image and caption.
5. Add the direct note URL as the first comment.
6. Verify the exact URL on `marcelozapata.dev` before sharing.
