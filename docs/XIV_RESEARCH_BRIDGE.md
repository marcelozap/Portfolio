...
On September 8, migrations0002 then0003 were applied once to the existing production project, each reporting success. Before application all new objects were absent, PostgreSQL17 supplied the required built-ins, and the schema owner matched the existing membership table. Post-application checks confirmed four new tables with RLS, exact owner/worker RPC execution grants, fixed function search paths, private-core/schema denial, no direct client writes and the bridge foreign key. The new tables are empty. Existing membership and note-table counts/digests were identical before and after. No owner token, synthetic task, capability or trading data was inserted. These catalog checks do not establish signed-in owner behavior, actual concurrent transactions or a live model exchange.
