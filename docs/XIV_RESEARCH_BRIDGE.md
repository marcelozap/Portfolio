...
Offline implementation includes migration `202609080003_xiv_research_bridges.sql`, the local Node transport, owner pairing controls, a Windows DPAPI vault and explicit session commands. No live capability has been issued, no owner has paired a session, and no production migration or deployment is established by these files. The website continues to say **No agent connected**. The protected storage base on this machine needs a deliberate resolution before actual pairing; live migration, publication and one real exchange remain unverified.
...
`src/lib/desk/bridge-vault.ts` uses the fixed `scripts/bridge-vault.ps1` helper through private redirected pipes. Credentials are encrypted with Windows CurrentUser DPAPI outside the checkout at `%LOCALAPPDATA%/XIV/research-bridge/<SHA-256 of exact session>/credential.dpapi`. Session and canonical file path bind the encryption. Each managed directory and file must have the current user as owner, protected inheritance, and only that user and SYSTEM with full access. Reparse paths, hard-linked credential files, unsafe roots, tampered ciphertext and expired identities are refused. A new ciphertext is flushed before an exclusive rename; an existing identity is never overwritten.
...
The production LocalAppData base currently grants write/delete access to additional Codex sandbox principals. The vault refuses that base. This increment does **not** alter those ACLs, initialize the real vault, or move production credentials to a weaker location. Tests explicitly use separate synthetic roots under Temp; only that test mode permits the shared Temp parent, while every managed vault directory/file still requires the private ACL. Passing those tests does not establish production storage readiness. Resolve the storage boundary deliberately before live pairing, without silently removing permissions needed by other applications.
...
Before calling the bridge connected: resolve the production storage base, verify the owner controls, apply only the new reviewed migrations, pair the actual existing session, and demonstrate one real request from the private website through claim/research/result back to the same task. Owner sign-in is still pending password recovery; independent offline implementation can proceed while Marcelo sleeps. Do not resend email or treat elapsed time as authorization. Add reviewed Gold-memory retrieval only after this basic exchange works, preserving decision-time versus retrospective evidence.
